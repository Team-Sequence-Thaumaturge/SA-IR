...
├─ 1부 ~ 3부: 인물 세포, 스타일링, 시공간 광학 물리 레이어 (LEVEL 01 ~ 09)
├─ 4부: 최종 출력 스타일 가변 제어 (Dynamic Profile 스위칭)
├─ LEVEL 12. 시네마틱 포스트-프로덕션 & VFX 컴포지팅 매트릭스
└─ LEVEL 13. 런타임 컴퓨트 워치독 (실시간 VRAM 지출 통제)
Repository Components
레포지토리는 사용자의 환경과 목적에 맞춰 두 가지 독립된 코어 스펙을 제공합니다.

SA-IR_PRO_v2.0.md: ACEScg 색역, ASC CDL 매트릭스, Z-Depth 패스 및 실시간 런타임 VRAM 워치독 제어권이 포함된 하이엔드 프로덕션 표준 명세서.

SA-IR_Flash_v2.0.md: 빠른 시각적 가속 및 범용 세션을 위한 경량화 다이어트 템플릿.

Quick Start: 원터치 인젝션 (One-Touch Injection)

설정: 본 레포지토리의 명세서 중 본인의 런타임 환경에 맞는 스펙 파일(PRO 혹은 Flash)을 선택하여 엽니다.

주입: 각 LEVEL의 슬롯을 세팅한 뒤, 최상단 시스템 컴파일러 스위치([⚠️ SYSTEM COMPILER SWITCH])부터 최후미 노드까지 문서 전체를 100% 복사합니다.

격발: DALL-E 3, Imagen 등 타깃 이미지 생성 커널 대화창에 그대로 주입(Paste & Run)하십시오. 상단 래퍼 주석이 LLM의 추론 전두엽을 제어하여 픽셀을 즉시 빌드해 냅니다.
